...
struct Poligono{
	PuntoR2* vertici; //puntatore a PuntoR2 per creare dinamicamente un array di punti
	unsigned int nvertici;
};

//Primo parametro: nome del file da cui caricare i dati
//Secondo parametro: puntatore a struct Poligono passato by reference per creare dinamicamente un array di poligoni
//Terzo parametro: intero in cui salvare il numero di poligoni caricati
void CaricaDaFile(const char* nomefile, Poligono* &poligoni, unsigned int &npoligoni);

bool Intrecciato(Poligono poligono); // funzione che determina se un poligono è intrecciato

void GetPunti(Poligono* poligoni, unsigned int npoligoni, PuntoR2* &punti, unsigned int &npunti); //Funzione per creare un array di punti contenente tutti i punti dei poligoni

void SelsortCrescente(PuntoR2 v[], unsigned int dim); //in ingresso un array e la sua dimensione
