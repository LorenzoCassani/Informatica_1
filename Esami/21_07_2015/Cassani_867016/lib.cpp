...
void CaricaDaFile(const char* nomefile, Poligono* &poligoni, unsigned int &npoligoni){
	...
	if(in.fail()){
		cerr << "Errore apertura file ''" << nomefile << "''. Chiudo il programma." << endl;
		exit(1);
	}
	...
	for(unsigned int i=0; i<npoligoni; ++i){
		in >> nvertici;
		poligoni[i].vertici = new PuntoR2[nvertici];
...
bool Intrecciato(Poligono poligono){
	...
	for(unsigned int i=0; i<poligono.nvertici-1; ++i){
		a1=(poligono.vertici[i].y-poligono.vertici[i+1].y)/(poligono.vertici[i].x-poligono.vertici[i+1].x); //m prima retta
		c1=poligono.vertici[i].y-poligono.vertici[i+1].x*a1; //q prima retta

		for(unsigned int j=i+2; j<poligono.nvertici-1; ++j){ //j=i+2: non considero il lato successivo
			a2=(poligono.vertici[j].y-poligono.vertici[j+1].y)/(poligono.vertici[i].x-poligono.vertici[i+1].x); //m seconda retta
			c2=poligono.vertici[j].y-poligono.vertici[j+1].x*a2; //q seconda retta

			if(a1==a2) continue; //le rette sono parallele, nessuna intersezione;
			...
			if((x_intersezione>poligono.vertici[i].x and
			    ...
			    x_intersezione>poligono.vertici[j].x)) return true;
		}
	}
	return false;
}
...
void GetPunti(Poligono* poligoni, unsigned int npoligoni, PuntoR2* &punti, unsigned int &npunti){
	...
	for(unsigned int i=0; i<npoligoni; ++i) npunti+=poligoni[i].nvertici; //conto i punti totali

	punti = new PuntoR2[npunti];

	unsigned int counter=0;

	for(unsigned int i=0; i<npoligoni; ++i){
		for(unsigned int j=0; j<poligoni[i].nvertici; ++j){
